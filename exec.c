...
void exec(FILE *arq){
   ...
   static unsigned int n_exec = 0; //  Contador de numeros de execução.
   ...
   short type_cod;   // Tipo de intstrução a ser executada, 0 = intstrução normal, 1 = intstrução chamada bloco.
...
void execinstr(char **vetline, FILE *arq){
    ...
    short fin = 0;   // Define se Fim de execução; 1==fim.
...
/*
    Executa instrução de chamada de blocos, empinha a o bloco atual.
*/
...
void print(int fin, FILE *arq){
    static int step = 0;   // Controle do modo -s
    step++;

    int dots;                   // Util pra criar o padrão de saida
    int fitantes,fitapos;       //   Util pra criar o padrão de saida
    short int flagvaziantes = 1;   //     Util pra criar o padrão de saida
    char fitaprint[46];   //         Util pra criar o padrão de saida
    char ponts[17];   //           Util pra criar o padrão de saida
    ...
    if (modo != 1) {  // Modo de impressão.
          ...
          if (flagvaziantes){  // Trata se tiver menos de 20 antes do cabecote.
              ...
              cont += cabecote;
          }else{  // Trata se tiver mais de 20 antes do cabecote.
             strcpy(fitaprint,"");
